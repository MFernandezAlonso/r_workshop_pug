...
3.2) Use `table()` on `hotdog_sex`. What do you get? --> tabulation
...
--> it gives mean, median, quartiles, no sd
...
--> first five elements, last five elements
...
hotdog_names[hotdog_sex=="female"][
  hotdog_2019[hotdog_sex=="female"] == max(hotdog_2019[hotdog_sex=="female"])
  ]
```

```{r}
which(hotdog_2018 == 74 | hotdog_2018 < 40)
```
